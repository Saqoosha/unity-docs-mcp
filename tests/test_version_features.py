...
class TestVersionFeatures(unittest.TestCase):
        ...
        with patch.object(self.server.scraper, 'get_latest_version') as mock_latest:
            ...
            with patch.object(self.server.scraper, 'validate_version') as mock_validate:
                ...
                with patch.object(self.server.scraper, 'get_api_doc') as mock_get_api:
                    ...
                    with patch.object(self.server.scraper, 'check_api_availability_across_versions') as mock_check:
                        mock_check.return_value = {"available": [], "unavailable": []}
                        
                        # Run async method
                        async def run_test():
                            result = await self.server._get_unity_api_doc("GameObject", None, None)
                            return result
    ...
    def test_version_normalization_in_api_doc(self):
        ...
        with patch.object(self.server.scraper, 'validate_version') as mock_validate:
            mock_validate.return_value = True
            with patch.object(self.server.scraper, 'normalize_version') as mock_normalize:
                ...
                with patch.object(self.server.scraper, 'get_api_doc') as mock_get_api:
                    ...
                    with patch.object(self.server.parser, 'parse_api_doc') as mock_parse:
                        ...
                        async def run_test():
                            result = await self.server._get_unity_api_doc("GameObject", None, "6000.0.29f1")
                            return result
            ...
            with patch.object(self.server.scraper, 'normalize_version') as mock_normalize:
                ...
                with patch.object(self.server.scraper, 'search_docs') as mock_search:
                    mock_search.return_value = {
                        "status": "success", 
                        "results": [{"title": "Transform", "url": "test.html", "description": "Test"}],
                        "count": 1
                    }
                    
                    async def run_test():
                        result = await self.server._search_unity_docs("transform", "2022.3.45f1")
                        return result
            ...
            with patch.object(self.server.scraper, 'normalize_version') as mock_normalize:
                ...
                with patch.object(self.server.scraper, 'get_api_doc') as mock_get_api:
                    ...
                    with patch.object(self.server.scraper, 'check_api_availability_across_versions') as mock_check:
                        ...
                        async def run_test():
                            result = await self.server._get_unity_api_doc("AsyncGPUReadback", None, "2019.4")
                            return result
                        ...
                        self.assertIn("**Available in versions:** 6000.0, 2022.3, 2021.3", content)
    ...
    def test_unsupported_version_with_normalization_info(self):
        ...
        with patch.object(self.server.scraper, 'validate_version') as mock_validate:
            ...
            with patch.object(self.server.scraper, 'normalize_version') as mock_normalize:
                ...
                with patch.object(self.server.scraper, 'get_supported_versions') as mock_supported:
                    mock_supported.return_value = ["6000.0", "2022.3", "2021.3"]
                    
                    async def run_test():
                        result = await self.server._get_unity_api_doc("GameObject", None, "1.0.5f1")
                        return result
                    
                    result = asyncio.run(run_test())
                    
                    content = result[0].text
                    self.assertIn("Unsupported Unity version '1.0.5f1' (normalized to '1.0')", content)
