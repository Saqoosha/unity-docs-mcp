...
class TestUnityDocsMCPServer(unittest.TestCase):
    ...
    async def test_get_unity_api_doc_success(self, mock_parser_class, mock_scraper_class):
    ...
    @patch('unity_docs_mcp.server.UnityDocScraper')
    @patch('unity_docs_mcp.server.UnityDocParser')
    async def test_get_unity_api_doc_parser_error(self, mock_parser_class, mock_scraper_class):
    ...
    @patch('unity_docs_mcp.server.UnityDocScraper')
    @patch('unity_docs_mcp.server.UnityDocParser')
    async def test_search_unity_docs_success(self, mock_parser_class, mock_scraper_class):
        ...
        mock_parser.parse_search_results.return_value = {
            "results": [
                {"title": "GameObject", "url": "https://docs.unity3d.com/GameObject.html", "description": "Base class"},
                {"title": "Transform", "url": "https://docs.unity3d.com/Transform.html", "description": "Position and rotation"}
            ],
            "count": 2
        }
    ...
    @patch('unity_docs_mcp.server.UnityDocScraper')
    @patch('unity_docs_mcp.server.UnityDocParser')
    async def test_search_unity_docs_no_results(self, mock_parser_class, mock_scraper_class):
    ...
    async def test_list_unity_versions(self, mock_scraper_class):
        ...
        mock_scraper = Mock()
        mock_scraper_class.return_value = mock_scraper
        mock_scraper.get_supported_versions.return_value = ["6000.0", "2023.3", "2022.1"]
    ...
    async def test_suggest_unity_classes(self, mock_scraper_class):
        ...
        mock_scraper = Mock()
        mock_scraper_class.return_value = mock_scraper
        mock_scraper.suggest_class_names.return_value = ["GameObject", "GameObjectUtility"]
