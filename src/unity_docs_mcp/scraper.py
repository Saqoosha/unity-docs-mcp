...
class UnityDocScraper:
        ...
        try:
            ...
            if "." not in class_name:
                search_results = self.search_index.search(
                    class_name, version, max_results=20
                )
            ...
            if method_name:
                if member_type:
                    ...
                    url = self._build_api_url(
                        actual_class_name, method_name, version, use_hyphen=use_hyphen
                    )
                else:
                    ...
                    if not html_content:
                        url = self._build_api_url(
                            actual_class_name, method_name, version, use_hyphen=True
                        )
                        html_content = self._fetch_page(url)
                    return (
                        {"url": url, "html": html_content, "status": "success"}
                        if html_content
                        else {
    ...
    def get_supported_versions(self) -> List[str]:
        ...
        try:
            ...
            response = self.session.get(
                "https://docs.unity3d.com/StaticFilesConfig/UnityVersionsInfo.js",
                timeout=10,
            )
            if response.status_code == 200:
                ...
                not_supported_match = re.search(
                    not_supported_pattern, content, re.DOTALL
                )
                ...
                if all_versions:
                    ...
                    sorted_versions = sorted(
                        all_versions, key=version_key, reverse=True
                    )
    ...
    def get_latest_version(self) -> str:
        ...
        try:
            ...
            response = self.session.get(
                "https://docs.unity3d.com/ScriptReference/GameObject.html", timeout=10
            )
            if response.status_code == 200:
    ...
    def suggest_class_names(self, partial_name: str) -> List[str]:
        ...
        suggestions = [cls for cls in common_classes if partial_lower in cls.lower()]
    ...
    def check_api_availability_across_versions(
        ...
        if "." not in class_name:
            ...
            search_results = self.search_index.search(
                class_name, test_versions[0], max_results=20
            )
            for result in search_results:
                if result.get("type") == "class":
                    url_parts = result["url"].split("/")
                    if url_parts:
                        ...
                        if filename.lower() == class_name.lower():
                            ...
                            "." in filename
                            and filename.split(".")[-1].lower() == class_name.lower()
                        ):
                            found_class_name = filename
                            break
        ...
        for version in test_versions:
            ...
            if self.validate_version(normalized_version):
                url = self._build_api_url(
                    actual_class_name, method_name, normalized_version
                )
        ...
        # Cache the result
        result = {"available": available_versions, "unavailable": unavailable_versions}
