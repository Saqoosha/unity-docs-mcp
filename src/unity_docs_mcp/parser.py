...
class UnityDocParser:
    ...
    def parse_api_doc(self, html_content: str, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if trafilatura_content:
                trafilatura_content = self._clean_trafilatura_content(
                    trafilatura_content
                )

            # Remove markdown formatting from the content (after other cleaning)
            if trafilatura_content:
                trafilatura_content = self._remove_markdown_formatting(
                    trafilatura_content
                )
    ...
    def _clean_trafilatura_content(self, content: str) -> str:
        ...
        content = re.sub(
            r"\[([A-Z][a-zA-Z0-9_]*)\]([a-z][a-zA-Z0-9_]*)", r"\1 \2", content
        )
        ...
        content = re.sub(
            r"=\s*\[([A-Z][a-zA-Z0-9_]*\.[A-Z][a-zA-Z0-9_]*)\]", r"= \1", content
        )
        ...
        content = re.sub(
            r"\+=\s*\[([A-Z][a-zA-Z0-9_]*\.[a-zA-Z][a-zA-Z0-9_]*)\]", r"+= \1", content
        )

        # Fix enum values: [PrimitiveType.Cube] -> PrimitiveType.Cube
        content = re.sub(
            r"\[([A-Z][a-zA-Z0-9_]*\.[A-Z][a-zA-Z0-9_]*)\]", r"\1", content
        )
    ...
    def _remove_unity_ui_elements(self, soup: BeautifulSoup) -> None:
        ...
        for element in soup.find_all(text=True):
            if any(
                pattern in element.string
                for pattern in feedback_text_patterns
                if element.string
            ):
